...
#Regular:
ggplot(data = coral_bleaching_events, mapping = aes(x = Entity, y = Moderate)) + 
  geom_boxplot()

#Flipped
ggplot(data = coral_bleaching_events, mapping = aes(x = Moderate, y = Entity)) + 
  geom_boxplot()
...
ggplot(data = coral_bleaching_events, (mapping = aes(x = Year, y = Total_bleaching_events, color = Entity))) + 
  geom_line() + 
  geom_point() +
  labs(title = "Bleaching Events Vs. Years",
       x = "Years",
       y = "Total Bleaching Events")
...
mar15 <- filter(flights, month == 3, day == 15)

### Good! Now run this code:
filter(flights, month = 1)
### Why did we get an error?

#We did not use the right comparative operator.  
#We used "=" instead of =
...
early_flights <- filter(flights, (sched_dep_time - dep_time == 10) & 
                          (sched_arr_time - arr_time == 10))
...
arrange(flights, dep_delay, desc(flight))
...
smaller_data_1 <- select(flights, year, month, day, dep_time)
